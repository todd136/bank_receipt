...
python -m nuitka --standalone --onefile --msvc=latest --assume-yes-for-downloads --noinclude-unittest-mode=nofollow --noinclude-pytest-mode=nofollow --enable-plugin=no-qt --low-memory --lto=no --jobs=2 --windows-uac-admin --onefile-tempdir-spec="{CACHE_DIR}/todd_dev_studio/bank_receipt" --windows-icon-from-ico=logo.ico --company-name="Todd Dev Studio" --product-name="BankReceiptParser" --file-version=1.0.0.0 --product-version=1.0.0.0 --file-description="用于分发银行回单的自动化工具" --copyright="Copyright (c) 2026 Todd Dev Studio. All rights reserved." --output-filename=bank_receipt src\bank_receipt\main.py
