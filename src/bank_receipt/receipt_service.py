...
def _clean_payer(raw: str) -> str:
    ...
    # 兼容误吸收“开户银行:”前缀（如“付款人开户银行”被宽泛规则命中）
    s = re.sub(r'^(开\s*户\s*银\s*行|开户银行)\s*[:：]\s*', '', s)
    # 防止把字段标签值（账号/卡号/开户行等）当作付款人名称
    if re.match(r'^(账\s*号|账号|卡\s*号|卡号|开\s*户\s*行|开户行|开\s*户\s*银\s*行|开户银行)\s*[:：]?', s):
        return ''
    if '收款人名称' in s:
    ...
    # 去掉后续拼接字段（常见于“整行连在一起”）
    for stop in ('账号', '账 号', '开户行', '开户银行', '币种', '用途', '金额', '附言', '摘要', '备注'):
